        ...
        .then(data => {
            ...
            function handleFiltersChange() {
                ...
                if (rangeInput && rangeInput.length > 1) {
                    selectedFilters.minPrice = parseInt(rangeInput[0].value, 10);
                    selectedFilters.maxPrice = parseInt(rangeInput[1].value, 10);

                    if (document.querySelector('.filter-price select')) {
                        const selectPrice = document.querySelector('.filter-price select').value;
                        if (selectPrice !== "null") {
                            const [min, max] = selectPrice.split('-').map(val => parseInt(val.replace('$', '').trim()), 10);
                            selectedFilters.minPrice = parseInt(min, 10);
                            selectedFilters.maxPrice = parseInt(max, 10);
                        } else {
